...
finishBtn.addEventListener("click", () => {
  ...
  if (balanceCount < numberValue) {
    ...
    balance.innerHTML = `${count.toLocaleString()} US$`;
    addTransaction();
    addUserTransaction();
  }
...
function addTransaction() {
  let transactionHTML = `
   <div class="recent-history">
            <div class="hist-img">
                <div class="sen-avatar" style="width: 66px; height: 100%; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 18px; color: white; background-color: ${generatedColor};">
          ${generatedInitial}
        </div>

              <div class="">
                <h3 class="pyp-balance">PayPal balance</h3>
                <p class="transfer-using user">@${input.value}</p>
                <p class="transfer-using">Instant transfer using Balance</p>
              </div>
            </div>
            <div class="transfer-count">
              <h4 class="sending">- $${inputUsd.value}.00 USD</h4>
            </div>
          </div>
  `;
